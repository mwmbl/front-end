...
<main class="flex w-full max-w-4xl flex-col gap-2 self-center px-6">
				...
				{:then submissionsForInput}
					{#if domainInput.length > 0}
						...
						{#if data.loginStatus == 'assumeLoggedIn'}
							<Button
								variant="secondary"
								class="dark:bg-muted mt-6 flex w-full flex-row items-center gap-2"
								type="submit"
							>
						...
						{:else}
							<div class="dark:bg-muted mt-6 flex w-full flex-row items-center gap-2">
	...
	<form class="mt-3 p-2">
		...
		<label class="flex flex-row items-center gap-4">
			By Status <Select.Root type="single" bind:value={filter.status}>
	...
	{@render submissions(
		data.submissions.items.filter((item) =>
			filter.status !== 'any' ? item.status == filter.status : true
		)
	)}
	<div class="flex flex-row items-center justify-center gap-4 p-4">
		...
		{#each Array.from({ length: data.maxPage + 1 }, (_, i) => i) as page}
			<Button
				variant={data.page == page ? 'default' : 'outline-solid'}
				size="icon"
				href="?page={page}"
			>
				{page}
